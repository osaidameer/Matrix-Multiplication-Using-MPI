   ...
   sleep(1);
	...
	for(int i=0; i<matrix_rows[0]; i++){
    	...
    	for(int i=0; i<line_counter - 1; i++){
			...
			MPI_Isend(&offset, 1, MPI_INT, j, 1, MPI_COMM_WORLD, &request);
			MPI_Isend(&rows, 1, MPI_INT, j, 1, MPI_COMM_WORLD, &request);
			MPI_Isend(&matrix_columns[i], 1, MPI_INT, j, 1, MPI_COMM_WORLD, &request);	
			MPI_Isend(&matrix_columns[i+1], 1, MPI_INT, j, 1, MPI_COMM_WORLD, &request);	
			...
			MPI_Isend(mA, rows * matrix_columns[i], MPI_INT, j, 1, MPI_COMM_WORLD, &request);	
			MPI_Wait(&request, MPI_STATUS_IGNORE);
			...
			MPI_Isend(mB, matrix_rows[i+1] * matrix_columns[i+1], MPI_INT, j, 1, MPI_COMM_WORLD, &request);	
			MPI_Wait(&request, MPI_STATUS_IGNORE);
		...
		//memset(matrixC, 0, sizeof(matrixC));
    		...
    		for(int j=1; j<=slave_count; j++){
    			...
    			MPI_Irecv(&offset, 1, MPI_INT, j, 2, MPI_COMM_WORLD, &request);
    			MPI_Irecv(&rows, 1, MPI_INT, j, 2, MPI_COMM_WORLD, &request);
    			MPI_Irecv(mC, rows * matrix_columns[i+1], MPI_INT, j, 2, MPI_COMM_WORLD, &request);
			MPI_Wait(&request, MPI_STATUS_IGNORE);
   	...
   	for (int i=0; i<line_counter - 1; i++){
   		...
   		MPI_Irecv(&rows, 1, MPI_INT, 0, 1, MPI_COMM_WORLD, &request);
   		MPI_Irecv(&c1, 1, MPI_INT, 0, 1, MPI_COMM_WORLD, &request); 
     		MPI_Irecv(&c2, 1, MPI_INT, 0, 1, MPI_COMM_WORLD, &request); 	
		MPI_Wait(&request, MPI_STATUS_IGNORE);
   		...
   		MPI_Irecv(mA, rows * c1, MPI_INT, 0, 1, MPI_COMM_WORLD, &request);
   		MPI_Irecv(mB, c1 * c2, MPI_INT, 0, 1, MPI_COMM_WORLD, &request);
		MPI_Wait(&request, MPI_STATUS_IGNORE);
   		...
   		MPI_Isend(&offset, 1, MPI_INT, 0, 2, MPI_COMM_WORLD, &request);
     		...
     		MPI_Isend(mR, rows * c2, MPI_INT, 0, 2, MPI_COMM_WORLD, &request); 		
     		MPI_Wait(&request, MPI_STATUS_IGNORE);	
   	}
   }
    ...
    if (rank==0){
   	...
   	fp1 = fopen("nonblocking.txt", "a");
	printf("\n\nTotal Execution time for non-blocking Multiplicaiton %f\n", t2 - t1);
